...
rf.train <- readRDS("~/Dropbox/PRUEBAS_DATA_SCIENCE/CoverWallet/CW_DS_challenge/challenge/Code_CW/output/model_forest.rds")
#rf.train <- readRDS(rf.train, file_name)

#convert, 
dt.rf  <- data  %>%  select(premium, annual_revenue ,ratio_carrier_winning, total_payroll, ratio_wins_products, 
...
pred.rf.train <- predict(rf.train, dt.rf, type = "prob")
...
write.table(data.def, file = 'solution/RF_SOLUTION.csv', sep = '|', quote = F, row.names = F)
...
write.table(val, file = 'solution/RF_account_value.csv', sep = '|', quote = F, row.names = F)
#--------------------------------------------------------------------------------------------------------------------------------
