...
dt.test  <- dt.test  %>%  select(convert, premium, annual_revenue ,ratio_carrier_winning, total_payroll,
                                 ratio_wins_products, Number_year_established,
                                 avg_carriers_winning_premium, Carriers_winning_premium_per_sell,
                                 Number_wins_products, num_employees, Carriers_winning_bids, Number_bids_products,
                                 Carriers_bids, sector_Food.and.Accommodation,
...
param <- list(
  ...
  "max_depth"        = 6, #5,  # OR 6 #learning rate, we start with 5 to learn
...
file_name <- "output/model_xgboost.rds"
saveRDS(xgb.model, file_name)
...
# cross.val <- cross.val$evaluation_log
# cat("[INFO] Cross validation mean: ", min(cross.val$test_auc_mean), "\n")
# cat("[INFO] Cross validation sd: ", min(cross.val$test_auc_std), "\n")
# #-------------------------------------------------------------------------------------------------------------------------------    
...
#var <- importance[1:20, ]
#saveRDS(var, file = 'output/names.xg.rds')
...
# ALL
d <- ggplot(importance[1:20, ], aes(y = Gain, x = reorder(Feature, Gain))) +  geom_bar(stat = "identity", color = 'blue') + coord_flip()
d <- d + ylab('Importance') + xlab('Features') 
d <- d + theme(axis.title.y = element_text(size = rel(1.5)))
d <- d + theme(axis.text.y = element_text(size = rel(1.8)))
d <- d + theme(axis.text.x = element_text(size = rel(1.8)))
